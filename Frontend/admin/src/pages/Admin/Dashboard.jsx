  ...
  return dashData && (
    <div className='m-5'>
      ...
      <div className='flex flex-wrap gap-3'>
        ...
        <div className='flex items-center gap-2 bg-white p-4 min-w-52 rounded border-2 border-gray-100 cursor-pointer hover:scale-105 transition-all'>
          <img className='w-14' src={assets.mentees_icon} alt="" />
          <div>
            <p className='text-xl font-semibold text-gray-600'>{dashData.mentees}</p>
            <p className='text-gray-400'>Mentees</p></div>
        </div>
      </div>
        ...
        <div className='pt-4 border border-t-0'>
          {dashData.latestAppointments.slice(0, 5).map((item, index) => (
            <div className='flex items-center px-6 py-3 gap-3 hover:bg-gray-100' key={index}>
              <img className='rounded-full w-10' src={item.mentData.image} alt="" />
